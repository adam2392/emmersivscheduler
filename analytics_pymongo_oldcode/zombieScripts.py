...
import os, pymongo, json, csv, time, types
...
def prioritizeCalc(db, userList, mode):
	...
	for users in userList:
		...
		userIndex += 1
	if len(prioritizeList) == 1:
		prioritizeList = prioritizeList[0]
...
def delibCalc(delibHits, delibShots, totalShots, weightHit, weightShot):
	...
	if isinstance(delibHits, NumberTypes):
		delibHits = {delibHits}	
	if isinstance(delibShots, NumberTypes):
		delibShots = {delibShots}
	if isinstance(totalShots, NumberTypes):
	...
	for index in xrange(0, len(delibHits)):
		...
		# if len(totalShots) == 1:
		# 	delibAttempts[index] = (round(float(total) / totalShots, 4))/maxWeight
		# else:
...
if __name__ == '__main__':
	...
	if isConnected == True:
		...
		userList = ['adam_good', 'adam_bad', 'adam_good01', 'adam_bad01']
		weightHit = 0.75
		weightShot = 0.25
